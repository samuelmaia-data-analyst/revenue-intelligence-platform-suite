...
def build_risk_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    recency_weight = (risk["Days Since Last Purchase"] / risk["Days Since Last Purchase"].max()).fillna(0)
    spend_weight = (risk["Total Spend"] / risk["Total Spend"].max()).fillna(0)
...
st.caption("Executive dashboard powered by real data from integrated modules.")
